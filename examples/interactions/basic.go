...
func main() {
	...
	// Use the default client options which read from environment variables.
	client, err := genai.NewClient(ctx, nil)
	if err != nil {
		log.Fatal(err)
	}

	// Interactions API is currently experimental and may not be supported on Vertex AI yet.
	...
	body := operations.NewCreateInteractionRequestBody(gaos_interactions.CreateModelInteraction{
		...
		Input: ptr(gaos_interactions.NewInteractionsInput("Why is the sky blue?")),
	})
