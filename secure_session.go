...
func (s *SecureRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if contents, ok := reqBody["contents"].([]any); ok {
		for _, c := range contents {
			if cMap, ok := c.(map[string]any); ok {
				if parts, ok := cMap["parts"].([]any); ok {
					for _, p := range parts {
						if pMap, ok := p.(map[string]any); ok {
							if _, hasInline := pMap["inlineData"]; hasInline {
								return nil, fmt.Errorf("private inference server error: Multimodal input via InlineData is not enabled for model %s", modelName)
							}
							if _, hasInline := pMap["inline_data"]; hasInline {
								return nil, fmt.Errorf("private inference server error: Multimodal input via InlineData is not enabled for model %s", modelName)
							}
						}
					}
	...
	case respBytes, ok := <-respChan:
		...
		if err := json.Unmarshal(respBytes, &m); err == nil {
			...
			if val, ok := m["error"]; ok {
				errVal = val
			} else if val, ok := m["status"]; ok {
				errVal = val
			} else if val, ok := m["privateInferenceError"]; ok {
				errVal = val
			}
			...
			if errVal != nil {
				...
				if gcResp != nil {
					gcRespCamel := convertKeysSnakeToCamel(gcResp)
					if unwrappedBytes, err := json.Marshal(gcRespCamel); err == nil {
