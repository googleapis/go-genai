...
import (
	...
	"google.golang.org/genai/internal/sentencepiece"
)

// geminiModelsToTokenizerNames maps model names to their tokenizer types
...
// geminiStableModelsToTokenizerNames maps stable model names to their tokenizer types
...
// getTokenizerName returns the tokenizer name for the given model name
func getTokenizerName(modelName string) (string, error) {
	if tokenizerName, ok := geminiModelsToTokenizerNames[modelName]; ok {
		return tokenizerName, nil
	}
	if tokenizerName, ok := geminiStableModelsToTokenizerNames[modelName]; ok {
		return tokenizerName, nil
	}

	// Build list of supported models for error message
	var supportedModels []string
	for model := range geminiModelsToTokenizerNames {
		supportedModels = append(supportedModels, model)
	}
	for model := range geminiStableModelsToTokenizerNames {
		supportedModels = append(supportedModels, model)
	}
...
func New(modelName string) (*Tokenizer, error) {
	tokenizerName, err := getTokenizerName(modelName)
	if err != nil {
...
// similar to the Python LocalTokenizer.count_tokens method.
func (tok *Tokenizer) CountTokens(contents []*genai.Content, config *genai.CountTokensConfig) (*genai.CountTokensResult, error) {
...
// similar to the Python LocalTokenizer.compute_tokens method.
func (tok *Tokenizer) ComputeTokens(contents []*genai.Content) (*genai.ComputeTokensResult, error) {
...
func (ta *textsAccumulator) addSchema(schema *genai.Schema) {
	...
	if schema.Enum != nil {
		for _, enumVal := range schema.Enum {
			ta.texts = append(ta.texts, enumVal)
		}
	}
