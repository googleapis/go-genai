...
func TestSendStreamRequest(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "Stream with SSE Comment Heartbeat",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   ": heartbeat\n\ndata:{\"key1\":\"value1\"}\n\n: heartbeat\n\ndata:{\"key2\":\"value2\"}\n\n",
			mockStatusCode: http.StatusOK,
			wantResponse: []map[string]any{
				{"key1": "value1"},
				{"key2": "value2"},
		...
		{
			name:           "Stream with SSE Comment Before Data In Same Block",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   ": heartbeat\ndata:{\"key1\":\"value1\"}\n\n",
		...
		{
			name:           "Stream with SSE Comment After Data In Same Block",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   "data:{\"key1\":\"value1\"}\n: heartbeat\n\n",
		...
		{
			name:           "Stream with Bare Colon Keepalive",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   ":\n\ndata:{\"key1\":\"value1\"}\n\n",
		...
		{
			name:           "Stream with Multi-Line Data",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   "data:{\ndata:\"key1\":\"value1\"\ndata:}\n\n",
		...
		{
			name:           "Stream with Leading Space After Colon",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   "data: {\"key1\":\"value1\"}\n\ndata:  {\"key2\":\"value2\"}\n\n",
			...
			wantResponse: []map[string]any{
				{"key1": "value1"},
				// Only one space is stripped; the second stays in the JSON.
				{"key2": "value2"},
			},
			wantErr: false,
		},
		{
			name:           "Stream with SSE Event Id and Retry Fields",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   "event: step.delta\nid: 42\nretry: 1000\ndata:{\"key1\":\"value1\"}\n\nretry: 1000\n\ndata:{\"key2\":\"value2\"}\n\n",
			mockStatusCode: http.StatusOK,
			wantResponse: []map[string]any{
				{"key1": "value1"},
				{"key2": "value2"},
		...
		{
			name:           "Successful Stream with Lone CR Newlines",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   "data:{\"key1\":\"value1\"}\r\rdata:{\"key2\":\"value2\"}\r\r",
			mockStatusCode: http.StatusOK,
			wantResponse: []map[string]any{
				{"key1": "value1"},
				{"key2": "value2"},
		...
		{
			name:           "Successful Stream with Leading Blank Lines",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   "\n\ndata:{\"key1\":\"value1\"}\n\n\r\rdata:{\"key2\":\"value2\"}\r\n\r\n",
		...
		{
			name:           "Stream without Trailing Blank Line",
			method:         "POST",
			path:           "test",
			body:           map[string]any{"key": "value"},
			mockResponse:   "data:{\"key1\":\"value1\"}",
		...
		{
			name:             "Stream with Valueless Data Field",
			method:           "POST",
			path:             "test",
			body:             map[string]any{"key": "value"},
			mockResponse:     "data\n\n",
			...
			wantResponse:     nil,
			wantErr:          true,
			wantErrorMessage: "iterateResponseStream: error unmarshalling data data:. error: unexpected end of JSON input",
		},
		{
			name:             "Stream with Single-Line Raw Error Body",
			method:           "POST",
			path:             "test",
			body:             map[string]any{"key": "value"},
			mockResponse:     "{\"error\":{\"code\":400,\"message\":\"raw error body\",\"status\":\"INVALID_ARGUMENT\"}}\n",
			mockStatusCode:   http.StatusOK,
			wantResponse:     nil,
			wantErr:          true,
			wantErrorMessage: "Error 400, Message: raw error body, Status: INVALID_ARGUMENT, Details: []",
		},
		{
			// A pretty-printed error body is not valid SSE and is not reassembled: every line
			// is reported as an invalid chunk, and callers see the first.
			name:             "Stream with Multi-Line Raw Error Body",
			method:           "POST",
			path:             "test",
			body:             map[string]any{"key": "value"},
			...
			wantResponse:     nil,
			wantErr:          true,
			wantErrorMessage: "iterateResponseStream: invalid stream chunk: {",
		},
		{
			name:             "Error Response",
			method:           "POST",
