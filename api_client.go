...
func (ac *apiClient) uploadFile(ctx context.Context, r io.Reader, uploadURL string, httpOptions *HTTPOptions) (*File, error) {
	...
	for {
		...
		// ErrUnexpectedEOF: reading fewer than len(buffer) bytes and encountered EOF.
		// EOF: Reading exactly len(buffer) bytes but the underlying reader return EOF at next call. In this case, original EOF is returned.
