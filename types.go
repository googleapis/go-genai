...
// Controls the maximum depth of the model's internal reasoning process before it produces
// a response. If not specified, the default is HIGH. Recommended for Gemini 3 or later
// models. Use with earlier models results in an error. This enum is not supported in
// Vertex AI.
...
const (
	// NULL value
	NULLValueNone NULLValue = "NULL_VALUE_NONE"
)
...
type PartialArg struct {
	// Optional. The NULL value of the partial argument.
	...
	// Optional. Required. A JSON Path (RFC 9535) to the argument being streamed. https://datatracker.ietf.org/doc/html/rfc9535.
	// e.g. "$.foo.bar[0].data".
...
type FunctionCall struct {
	...
	// Optional. The partial arguments of the function call. This is only used for function
	// call that is streamed out in multiple parts/contents/responses.
...
type FunctionCallingConfig struct {
	...
	// in the [FunctionCall.partial_args] field.
...
type ThinkingConfig struct {
	...
	// it produces a response. If not specified, the default is HIGH. Recommended for Gemini
	// 3 or later models. Use with earlier models results in an error. This field is not
...
// user with rich, location-based information.
type GroundingChunkMaps struct {
	// Optional. The sources that were used to generate the place answer. This includes
	// review snippets and photos that were used to generate the answer, as well as URIs
	// to flag content.
	PlaceAnswerSources *GroundingChunkMapsPlaceAnswerSources `json:"placeAnswerSources,omitempty"`
	// Optional. This Place's resource name, in `places/{place_id}` format. This can be
...
type GroundingChunk struct {
	// Optional. A `Maps` chunk is a piece of evidence that comes from Google Maps. It contains
	// information about a place, such as its name, address, and reviews. This is used to
...
type Segment struct {
	// Optional. Output only. Start index in the given Part, measured in bytes. Offset from
	// the start of the Part, inclusive, starting at zero.
	StartIndex int32 `json:"startIndex,omitempty"`
	// Optional. Output only. End index in the given Part, measured in bytes. Offset from
	// the start of the Part, exclusive, starting at zero.
	EndIndex int32 `json:"endIndex,omitempty"`
	// Optional. Output only. The index of a Part object within its parent Content object.
	PartIndex int32 `json:"partIndex,omitempty"`
	// Optional. Output only. The text corresponding to the segment from the response.
	Text string `json:"text,omitempty"`
}
...
type GroundingSupport struct {
	...
	// Optional. A list of indices (into 'grounding_chunk') specifying the citations associated
	// with the claim. For instance [1,3,4] means that grounding_chunk[1], grounding_chunk[3],
...
type RetrievalMetadata struct {
	...
	// the prompt. The score is in the range [0, 1], where 0 is the least likely and 1 is
	// the most likely. This score is only populated when google search grounding and dynamic
...
type GroundingMetadata struct {
	// Optional. List of supporting references retrieved from specified grounding source.
...
type Candidate struct {
	...
	// Optional. Output only. Grounding metadata for the candidate. This field is populated
	// for `GenerateContent` calls.
