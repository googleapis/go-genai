...
func (m Operations) fetchPredictOperation(ctx context.Context, operationName string, resourceName string, config *FetchPredictOperationConfig) (*GenerateVideosOperation, error) {
	parameterMap := make(map[string]any)
	...
	if m.apiClient.clientConfig.Backend == BackendGeminiAPI {
		return nil, fmt.Errorf("method FetchPredictOperation is only supported in the Vertex AI client. You can choose to use Vertex AI by setting ClientConfig.Backend to BackendVertexAI.")
	}
	...
	if m.apiClient.clientConfig.Backend == BackendVertexAI {
		...
		toConverter = fetchPredictOperationParametersToMldev
		fromConverter = generateVideosOperationFromMldev
	}
...
var experimentalWarningOperationsGet sync.Once

func (m Operations) Get(ctx context.Context, operation *GenerateVideosOperation, config *GetOperationConfig) (*GenerateVideosOperation, error) {
	experimentalWarningOperationsGet.Do(func() {
		log.Println("Warning: The method Operations.Get is experimental and may change in future versions.")
	})
	...
	if m.apiClient.clientConfig.Backend == BackendVertexAI {
		...
		if len(parts) > 1 {
			...
			return m.fetchPredictOperation(ctx, operationName, resourceName, fetchPredictOperationConfig)
		} else {
